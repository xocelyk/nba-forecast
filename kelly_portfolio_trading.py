...
def calculate_kelly_portfolio_weights(
    model_probs: pd.Series, fair_market_probs: pd.Series
) -> pd.Series:
    """
...
class KellyPortfolioTrader:
    ...
    def update_position_prices(
        self, fair_market_probs: pd.Series, team_mapping: Dict[str, str]
    ):
        """Update current prices for all positions."""
        for team, position in self.positions.items():
            full_name = team_mapping.get(team, team)
            if (
                full_name in fair_market_probs.index
                and fair_market_probs[full_name] > 0
            ):
    ...
    def rebalance_portfolio(
        ...
        kelly_weights = calculate_kelly_portfolio_weights(
            aligned_model_probs, aligned_market_probs
        )
        ...
        for team, weight in significant_weights.sort_values(
        ...
        for team in eliminated_teams:
            ...
            if position.shares != 0:
                print(
                    f"  🚫 {team} eliminated - closing position ({position.shares:+.1f} shares)"
                )
                # Position expires worthless
                pnl = -position.shares * position.avg_entry_price
                self.cash += (
                    pnl if position.shares < 0 else 0
                )  # Return margin for short positions
                trades_executed.append(
                    f"CLOSE {position.shares:+.1f} {team} (eliminated)"
                )
        ...
        print(
            f"Cash: ${self.cash:.2f} | Gross Exposure: {self.get_gross_exposure()/final_portfolio_value:.1%}"
        )
        ...
        if not self.positions:
            ...
            print(
                f"{'Team':4} {'Position':>8} {'Shares':>10} {'Price':>7} {'Value':>10} {'P&L':>10} {'% Port':>7}"
            )
            print("-" * 80)
            ...
            for team, pos in sorted_positions:
                ...
                print(
                    f"{team:4} {position_type:>8} {pos.shares:>10.1f} {pos.current_price:>7.3f} "
            ...
            print("-" * 80)
            print(
                f"{'Total Long:':>35} ${total_long_value:>9.2f} {(total_long_value/final_portfolio_value)*100:>15.1f}%"
            )
            print(
                f"{'Total Short:':>35} ${total_short_value:>9.2f} {(total_short_value/final_portfolio_value)*100:>15.1f}%"
            )
            print(
                f"{'Net Exposure:':>35} ${total_long_value - total_short_value:>9.2f} "
                f"{((total_long_value - total_short_value)/final_portfolio_value)*100:>15.1f}%"
            )
            print(
                f"{'Cash:':>35} ${self.cash:>9.2f} {(self.cash/final_portfolio_value)*100:>15.1f}%"
            )
        ...
        if shares > 0:  # Buying (going long or covering short)
            ...
            if shares > 0:  # Additional long position
                ...
                if position.shares >= 0:
                    ...
                    total_cost = (
                        position.shares * position.avg_entry_price + shares * price
                    )
                    position.shares += shares
                    position.avg_entry_price = (
                        total_cost / position.shares if position.shares > 0 else price
                    )
                else:
                    position.shares += shares
            ...
            if shares > 0:  # Short position
                ...
                if position.shares <= 0:
                    ...
                    total_margin = (
                        abs(position.shares) * position.avg_entry_price + shares * price
                    )
                    position.shares -= shares
                    position.avg_entry_price = (
                        total_margin / abs(position.shares)
                        if position.shares != 0
                        else price
                    )
                else:
        ...
        for team, position in self.positions.items():
            if team == winner_team:
                ...
                if position.shares > 0:  # Long position on winner
                    ...
                    total_pnl += pnl
                    print(
                        f"  {team}: LONG {position.shares:.1f} shares @ ${position.avg_entry_price:.3f} → $1.00 = ${pnl:+.2f} ✅"
                    )
                else:  # Short position on winner (we lose)
                    # We received entry price but must pay $1 per share
                    pnl = position.shares * (
                        position.avg_entry_price - 1.0
                    )  # shares are negative
                    total_pnl += pnl
                    print(
                        f"  {team}: SHORT {abs(position.shares):.1f} shares @ ${position.avg_entry_price:.3f} → $1.00 = ${pnl:+.2f} ❌"
                    )
            else:
                ...
                if position.shares > 0:  # Long position loses
                    ...
                    total_pnl += pnl
                    print(
                        f"  {team}: LONG {position.shares:.1f} shares @ ${position.avg_entry_price:.3f} → $0.00 = ${pnl:+.2f} ❌"
                    )
                else:  # Short position wins
                    # We received entry price, pay nothing
                    pnl = (
                        -position.shares * position.avg_entry_price
                    )  # shares are negative, so this is positive
                    total_pnl += pnl
                    print(
                        f"  {team}: SHORT {abs(position.shares):.1f} shares @ ${position.avg_entry_price:.3f} → $0.00 = ${pnl:+.2f} ✅"
                    )
...
def main():
    ...
    parser.add_argument(
        "--model-data", type=str, default="aligned_model_predictions_2025.csv"
    )
    parser.add_argument("--starting-capital", type=float, default=1000.0)
    parser.add_argument(
        "--kelly-fractions", nargs="+", type=float, default=[0.1, 0.25, 0.5, 1.0]
    )
    ...
    try:
        ...
        available_dates = [
            d for d in date_order if d in model_data["market_date_name"].unique()
        ]
        ...
        for kelly_frac in args.kelly_fractions:
            ...
            trader = KellyPortfolioTrader(
                starting_capital=args.starting_capital, kelly_fraction=kelly_frac
            )
            ...
            for date in available_dates:
                ...
                trader.rebalance_portfolio(
                    date, model_probs, fair_market_probs, team_mapping
                )
        ...
        for kelly_frac, result in results.items():
            print(
                f"{kelly_frac:4.1f}x Kelly: ${result['final_value']:7.2f} ({result['total_return']:+5.1f}%) | "
