...
class TestGetTeamNames:
    ...
    def test_get_team_names_duplicate_teams(self, mock_schedule):
        ...
        mock_game2 = MagicMock()
        mock_game2.dataframe = pd.DataFrame({
            'opponent_name': ['Boston Celtics'],  # Duplicate
            'opponent_abbr': ['BOS']
        })
...
class TestLoadYearData:
    ...
    def test_load_year_data_basic(self, mock_data_dir):
        ...
        expected_columns = [
            'boxscore_id', 'date', 'team', 'opponent', 
            'team_score', 'opponent_score', 'location', 'pace', 'completed', 'year'
        ]
...
class TestUpdateData:
    ...
    def test_update_data_basic(self, mock_sleep, mock_schedule, mock_boxscore, mock_data_dir):
        ...
        expected_columns = [
            'date', 'team', 'opponent', 'team_name', 'opponent_name',
            'team_score', 'opponent_score', 'margin', 'location', 'pace', 'completed', 'year'
        ]
    ...
    @patch('data_loader.Schedule')
    def test_update_data_with_preload(self, mock_schedule, mock_load_year, mock_data_dir):
        """Test data updating with preloaded data."""
        # Mock preloaded data
        mock_load_year.return_value = [
            ['202501010BOS', pd.Timestamp('2025-01-01'), 'BOS', 'LAL', 110, 108, 'Home', 98.5, True, 2025]
        ]
...
class TestLoadTrainingData:
    ...
    def _create_comprehensive_win_totals_data():
        ...
        all_teams = ['ATL', 'BOS', 'BRK', 'CHA', 'CHI', 'CHO', 'CLE', 'DAL', 'DEN', 'DET', 
                     'GSW', 'HOU', 'IND', 'LAC', 'LAL', 'MEM', 'MIA', 'MIL', 'MIN', 'NJN', 
                     'NOH', 'NOP', 'NYK', 'OKC', 'ORL', 'PHI', 'PHO', 'POR', 'SAC', 'SAS', 
                     'TOR', 'UTA', 'WAS']
        win_totals_data = {}
        for year in range(2010, 2026):  # Cover all possible years
            win_totals_data[str(year)] = {team: 45.0 + (hash(team + str(year)) % 20) for team in all_teams}
    ...
    def test_load_training_data_without_update(self, mock_data_dir):
        ...
        with patch('data_loader.load_regular_season_win_totals_futures') as mock_win_totals:
            ...
            with patch('data_loader.env.DATA_DIR', str(mock_data_dir)):
                with patch('data_loader.utils.load_hca_map') as mock_hca_load:
                    ...
                    with patch('data_loader.add_days_since_most_recent_game') as mock_add_days:
                        mock_add_days.return_value = training_data
                        
                        result = data_loader.load_training_data(
                            names=['BOS', 'LAL', 'MIA', 'DEN'],
                            update=False
                        )
            ...
            with patch('data_loader.env.DATA_DIR', str(mock_data_dir)):
                with patch('data_loader.utils.load_hca_map') as mock_hca_load:
                    ...
                    with patch('data_loader.add_days_since_most_recent_game') as mock_add_days:
                        ...
                        result = data_loader.load_training_data(['BOS', 'LAL'], update=False)
        ...
        with patch('data_loader.load_regular_season_win_totals_futures') as mock_win_totals:
            ...
            with patch('data_loader.env.DATA_DIR', str(mock_data_dir)):
                with patch('data_loader.utils.load_hca_map') as mock_load_hca:
                    mock_load_hca.return_value = {}  # Empty - should trigger calculation
                    
                    with patch('data_loader.utils.calculate_hca_by_season') as mock_calc_hca:
                    ...
                    with patch('data_loader.utils.load_hca_map') as mock_hca_load:
                        ...
                        with patch('data_loader.add_days_since_most_recent_game') as mock_add_days:
                            ...
                            def mock_add_days_func(df, cap=10):
                                ...
                                result_df['opponent_days_since_most_recent_game'] = [2, 1]
                                return result_df
