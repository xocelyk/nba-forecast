...
class TestDuplicateGames:
    ...
    def test_duplicate_games_team_opponent_swap(self, sample_games_data):
        ...
        assert (
            duplicated_games["team"].values == original_games["opponent"].values
        ).all()
        assert (
            duplicated_games["opponent"].values == original_games["team"].values
        ).all()
    ...
    def test_duplicate_games_margin_adjustment(self, sample_games_data):
        ...
        np.testing.assert_array_almost_equal(
            duplicated_games["margin"].values, expected_margins.values
        )
    ...
    def test_duplicate_games_team_win_flip(self, sample_games_data):
        ...
        np.testing.assert_array_equal(
            duplicated_games["team_win"].values, expected_team_win.values
        )
...
class TestDaysSinceMostRecentGame:
    ...
    def test_days_since_most_recent_game_basic(self):
        ...
        result = utils.days_since_most_recent_game(
            "BOS", target_date, games, cap=10, hca=2.5
        )
